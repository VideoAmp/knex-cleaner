...
var DefaultOptions = {
  ...
  postgresSchemas: ['public']   // List of tables to not delete
};

function getTablesNameSql(knex, options) {
  var client = knex.client.dialect;
  var databaseName = knex.client.databaseName ||
  knex.client.connectionSettings.database;

  var postgresSchemaNames = _.chain(options.postgresSchemas)
    .map(function(schema) {
  ...
  switch(client) {
    ...
    case 'postgresql':
      return "SELECT schemaname || '.' || tablename AS tablename " +
      "FROM pg_catalog.pg_tables " +
      "WHERE " + postgresSchemaNames + ";";
    case 'sqlite3':
...
function getTableNames(knex, options) {
  ...
  return knex.raw(getTablesNameSql(knex, options))
    .then(function(resp) {
